...
import { 
    ...
    Plus, Trash2, Edit2, Key, Shield, DollarSign, AlertCircle, TrendingUp,
    Calendar, Package, BarChart3, Download, Bell, Clock, Trash
} from 'lucide-react';
...
const AdminSettings = () => {
    const [settings, setSettings] = useState({
        smtp_email: '', smtp_password: '', smtp_host: '', smtp_port: 587, smtp_secure: false, 
        report_email: '', auto_billing_enabled: false, auto_expiry_enabled: false
    });
    const [loading, setLoading] = useState(true);
    const [billingSummary, setBillingSummary] = useState({ eligibleToday: [], recentLogs: [] });

    const fetchSummary = () => {
        api.get('/admin/billing-summary').then(res => setBillingSummary(res.data)).catch(err => console.error(err));
    };
    ...
    const triggerBilling = async () => {
        if (!confirm("Deseja disparar emails de cobrança para clientes em atraso agora (respeitando a regra de 7 dias)?")) return;
        try {
            const { data } = await api.post('/admin/trigger-billing');
            alert(data.message || "Processo iniciado.");
            fetchSummary();
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            <div className="card" style={{ padding: '1.5rem' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                    <div>
                        <label style={{ display: 'block', fontSize: '0.8rem', marginBottom: '0.25rem' }}>Servidor SMTP (Host)</label>
                        <input
                            type="text"
                            className="input-field"
                    ...
                    <div>
                        <label style={{ display: 'block', fontSize: '0.8rem', marginBottom: '0.25rem' }}>Porta</label>
                        <input
                            type="number"
                            className="input-field"
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                    <div>
                        <label style={{ display: 'block', fontSize: '0.8rem', marginBottom: '0.25rem' }}>E-mail do Servidor (Usuário)</label>
                        <input
                            type="email"
                            className="input-field"
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem' }}>
                    ...
                    />
                    <label htmlFor="smtp_secure" style={{ fontSize: '0.85rem', cursor: 'pointer' }}>Usar Conexão Segura (SSL/TLS - Geralmente Porta 465)</label>
                </div>
            ...
            <div style={{ 
                ...
                <div className="card" style={{ padding: '1.5rem' }}>
                    <h3 style={{ marginBottom: '1rem', fontSize: '1rem', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        <Clock size={18} /> Fila de Cobrança (Elegíveis Hoje)
                    </h3>
                    <div className="table-container" style={{ maxHeight: '300px', overflowY: 'auto' }}>
                        <table style={{ fontSize: '0.8rem' }}>
                            <thead><tr><th>Cliente</th><th>Vencimento</th></tr></thead>
                            <tbody>
                                {billingSummary.eligibleToday.length === 0 ? (
                                    <tr><td colSpan="2" style={{ textAlign: 'center', padding: '1rem' }}>Nenhum cliente na fila.</td></tr>
                                ) : billingSummary.eligibleToday.map((c, i) => (
                                    <tr key={i}>
                                        <td style={{ fontWeight: 600 }}>{c.client_name}</td>
                                        <td>{format(new Date(c.dueDate), 'dd/MM/yyyy')}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                ...
                <div className="card" style={{ padding: '1.5rem' }}>
                    <h3 style={{ marginBottom: '1rem', fontSize: '1rem', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        <History size={18} /> Log de Envios SMTP (Últimos 50)
                    </h3>
                    <div className="table-container" style={{ maxHeight: '300px', overflowY: 'auto' }}>
                        <table style={{ fontSize: '0.75rem' }}>
                            <thead><tr><th>Destinatário</th><th>Horário</th><th>Status</th></tr></thead>
                            <tbody>
                                {billingSummary.recentLogs.length === 0 ? (
                                    <tr><td colSpan="3" style={{ textAlign: 'center', padding: '1rem' }}>Nenhum envio registrado.</td></tr>
                                ) : billingSummary.recentLogs.map((l, i) => (
                                    <tr key={i}>
                                        <td title={l.subject}>{l.recipient}</td>
                                        <td>{format(new Date(l.created_at), 'HH:mm:ss')}</td>
                                        <td>
                                            <span style={{ 
                                                color: l.status === 'Enviado' ? '#10b981' : '#ef4444', 
                                                fontWeight: 700 
