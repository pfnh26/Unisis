...
services.admin = new AdminService(pool, services.notification, services.commission);
...
const runAutoBilling = async () => {
    try {
        ...
        if (hour >= 8 && hour < 18) {
            const { overdue } = await services.notification.getNotificationSummary();
            if (overdue.length > 0) {
                const client = overdue[Math.floor(Math.random() * overdue.length)];
                if (client.client_email) {
                    await services.notification.sendEmail(client.client_email, "Aviso de Cobrança - UniSis", `
                        <h2>Olá, ${client.client_name}</h2>
                        <p>Consta em nosso sistema uma fatura pendente com vencimento em ${format(client.dueDate, 'dd/MM/yyyy')}.</p>
