...
class AdminService {
    constructor(pool, notificationService, commissionService, pdfService) {
        this.pool = pool;
    ...
    async triggerBilling() {
        const { overdue } = await this.notificationService.getNotificationSummary({ role: 'Administrador' });
        
        // Filter those who haven't received an email in the last 7 days
        ...
        if (eligibleClients.length === 0) {
            return { message: "Nenhum cliente elegível para cobrança no momento (regra de 7 dias)." };
        }

        // Logic for background sending
        const sendWithDelays = async () => {
            console.log(`Starting background billing with attachments for ${eligibleClients.length} clients...`);
            for (const client of eligibleClients) {
                try {
                    // Fetch ALL overdue invoices for this client
                    const overdueInvoices = await this.notificationService.getOverdueInvoicesForClient(client.client_id);
                    const attachments = [];
                    ...
                    await this.notificationService.sendEmail(
                        client.client_email, 
                        "Aviso de Cobrança - UniSis (Faturas em Anexo)", 
                        `
                            <h2>Olá, ${client.client_name}</h2>
                            <p>Consta em nosso sistema que você possui <b>${overdueInvoices.length}</b> fatura(s) pendente(s).</p>
                            <p>As faturas detalhadas seguem em anexo para conferência e pagamento.</p>
                            <p>Por favor, realize o acerto o quanto antes para evitar a suspensão dos serviços contratados.</p>
                            <p>Atenciosamente,<br>Equipe UniSis</p>
                        `,
                        attachments
                    );
                    await this.notificationService.updateLastBillingTimestamp(client.client_id);
                    
                    // Wait between 30s and 3m
                    const delay = Math.floor(Math.random() * (180000 - 30000 + 1) + 30000);
                    console.log(`Email with ${attachments.length} attachments sent to ${client.client_name}. Next in ${delay/1000}s...`);
                    await new Promise(resolve => setTimeout(resolve, delay));
                } catch (err) {
                    console.error(`Error sending billing to ${client.client_name}:`, err.message);
                }
            }
            console.log("Background billing process finished.");
        };

        // Start background process without awaiting it to avoid request timeout
        sendWithDelays();

        return { message: `Processo de cobrança iniciado para ${eligibleClients.length} clientes. Eles receberão e-mails em intervalos aleatórios (30s a 3m).` };
    }
    ...
    async getBillingSummary() {
        // 1. Get Eligible Today (Logic from triggerBilling)
        const { overdue } = await this.notificationService.getNotificationSummary({ role: 'Administrador' });
        const sevenDaysAgo = new Date();
        sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);

        const eligible = overdue.filter(client => {
            if (!client.client_email) return false;
            if (!client.last_billing_email_at) return true;
            return new Date(client.last_billing_email_at) < sevenDaysAgo;
        });

        // 2. Get Recent Logs (Last 50)
        const logsRes = await this.pool.query("SELECT * FROM email_logs ORDER BY created_at DESC LIMIT 50");
